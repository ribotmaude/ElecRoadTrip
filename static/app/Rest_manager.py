...
# IMPORT
# =============================================================================
from Client_Rest import Client_Rest 
...
# =============================================================================
    def get_bornes_address(self, depart_add, arriv_add, autonomie):
        ...
        coord_d_add = self.client_rest.get_coordinates(depart_add)
        coord_a_add = self.client_rest.get_coordinates(arriv_add)
        
        #récupération de la distance entre les 2 adresses 
        total_distance = self.client_rest.get_distance(coord_d_add,coord_a_add)
        
        #récupération du nombre d'arret 
        nbr_stop = self.client_rest.get_nbr_stop(total_distance,autonomie)
        
        #récupération des distances d'arrets
        list_stop = self.client_rest.get_coodinates_end_auto(coord_d_add,coord_a_add,nbr_stop)
        
        #récupération des coordonnées bornes d'arrets 
        coord_bornes_stop = self.client_rest.get_bornes(list_stop,1000)
        
        #récupération des adresses des bornes d'arrets 
        adds_bornes_stop = []
        
        for bornes in coord_bornes_stop:
            add_borne_stop = self.client_rest.get_address(bornes[0],bornes[1])
...
# print (rest_manager.get_bornes_address(adr1, adr2, 140))
# print (rest_manager.get_nbr_stop(adr1, adr2,140))
